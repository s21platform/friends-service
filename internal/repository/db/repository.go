...
func (r *Repository) GetCountFriends(uuid string) (int64, int64, error) {
	...
	err := r.connection.Get(&subscription, "SELECT count(initiator) FROM friends WHERE user_id = $1", uuid)

	if err != nil {
		return 0, 0, fmt.Errorf("subscription r.connection.Select: %v", err)
	}

	err = r.connection.Get(&subscribers, "SELECT count(user_id) FROM friends WHERE initiator = $1", uuid)
